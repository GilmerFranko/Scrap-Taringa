    ...
    function getSession()
    {
        ...
        if (isset($_COOKIE[$this->config['cookie_name']]))
        {
            ...
            if ($query == true && $query->num_rows > 0)
            {
                ...
                // ¿ES VIP?
                $this->is_vip = $this->memberData['vip'] > time() ? true : false;
                // CóDIGO DE SEGURIDAD
                $this->token = md5('token' . $this->memberData['session']);
                // ¿ES MODERADOR - ADMINISTRADOR?
                $this->is_admod = $this->getLevel();
                ...
                // GANANCIAS
               //OCULTE ESTO if($this->memberData['pp_gender'] == 1)
                {
                    $this->memberData['earnings'] = $this->config['shouts_earnings_downloads']*$this->memberData['shouts_downloads']/100/100;
                }
