...
class Extra extends Model
{
    ...
    * @return string
    */
    public function curtText($input = null, $half = false ,$limit = 128)
    {
      if($half == false)
      {
      ...
      {
        $cHalf = ceil(strlen($input) / 2);
        if($cHalf > 800)
        {
        $input=substr($input, 0, $cHalf);
        $input = $input . "...";
        return $input;
        }
        else
    ...
    */
    public function getFirstImgHtml($html = null, $returnUrl = false)
    {
        ...
        // GENERO UN OBJETO HTML
        $doc    =   new DOMDocument();
        // INSTANCIO EL OBJETO CON EL STRING PASADO
        @$doc->loadHTML($html);

        // SELECCIONO TODOS LOS OBJETOS DENTRO DE LA ETIQUETA IMG
        $imgs = $doc->getElementsByTagName('img');
        // SELECCIONO TODOS LOS OBJETOS DENTRO DE LA ETIQUETA IFRAME
        $iframes = $doc->getElementsByTagName('iframe');
        // SELECCIONO TODOS LOS OBJETOS DENTRO DE LA ETIQUETA VIDEO
        ...
        // SI EXISTE UN VIDEO /*EN REVICIÓN*/
        else:
